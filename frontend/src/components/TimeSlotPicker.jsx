      ...
      {slots.map((slot) => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-1.5 py-2.5 px-4 rounded-xl text-sm font-medium border transition-all
              ${isDisabled
                ? 'bg-gray-50 text-gray-300 border-gray-100 cursor-not-allowed line-through'
                : isSelected
                  ? 'bg-teal-600 text-white border-teal-600 shadow-sm shadow-teal-200'
                  : 'bg-white text-gray-700 border-gray-200 hover:border-teal-400 hover:text-teal-600 hover:bg-teal-50'
              }`}
          >
            <Clock size={13} />
